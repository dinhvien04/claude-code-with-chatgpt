...
<div class="card">
  ...
  <p class="hint">The pairing code was generated on this computer.<br>It expires in a few minutes.</p>
</div>
</body>
</html>`;
}
  ...
  router.post("/oauth/authorize", urlencoded({ extended: false }), (req, res) => {
    ...
    if (!verdict.ok) {
      const messages: Record<string, string> = {
        ...
        expired: "This pairing code has expired. Run `c2c pair` for a fresh code.",
        too_many_attempts: "Too many incorrect attempts. Run `c2c pair` for a fresh code.",
        rate_limited: "Too many attempts. Please wait a minute and try again.",
        no_active_session: "No active pairing session. Run `c2c pair` to generate a pairing code.",
      };
